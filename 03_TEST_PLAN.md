...
- **Unit**: 실행일·공휴일·다음 창·T-3분/T/+3분 경계와 상태 전이를 순수 계산으로 검증한다.
...
표준 자동 시나리오는 첫 T=`09:00`, 사전 점검=`08:57`, 재시도 마감=`09:03`, 가상 window=`5분`, 선택 요일=월~금이다.
...
- 기대: 첫 1회와 후속 2회만 대상이 된다. 각 창은 T-3분 점검, T 직후 확인, 필요 시 호출 순서이며 세 창이 모두 비활성이면 CLI 호출은 정확히 3회다. 네 번째 예약/호출은 없고 다음 실행일에는 당일 상태가 초기화된다.
...
- 실행: T-3분/T 직후/수동 워밍에서 quota provider가 이미 새 창을 반환하게 한다.
...
- 실행: quota 반영 지연, CLI 실패/hang, timer·수동 이벤트 동시 도착, 실행 중 앱 재시작을 주입한다.
- 기대: 재확인은 T~+3분 안의 정해진 횟수로 끝난다. 동일 창의 PTY 프로세스는 동시에 하나뿐이고 전체 호출은 설정된 retry 상한 이하다. warm 성공 후 quota 반영이 늦는 경우에는 CLI를 다시 호출하지 않는다. +3분 뒤에는 실패로 종료하고 catch-up하지 않는다.
- 증거: retry 종료 이유, 최대 동시 프로세스=1, 호출/조회 횟수≤설정 상한, +3분 이후 호출 0회, orphan process 없음.

### TC-OS-001 — 잠금 지원과 sleep no-catch-up (P0, Integration+macOS)

- 참조: `REQ-011`; `FLOW-009`, `FLOW-010`
- 실행: (a) system sleep을 막고 화면만 잠근 상태에서 예약을 확인한다. (b) fake clock으로 T+3분 이후 복귀를 재현한다.
- 기대: (a)는 예약과 결과 기록이 정상 동작한다. (b)는 `놓침`으로 표시하고 복귀 직후 실행하지 않는다.
- 증거: 잠금 상태 앱 로그와 (b)의 복귀 후 spawn-count=0. 실제 sleep·덮개·종료·로그아웃 동작 보장은 테스트하지 않는다.
...
- 기대: 오늘 정지는 오늘 남은 미시작 자동 창을 막는다. 다음 건너뛰기는 다음 한 건을 건너뛴 뒤 실제 `resets_at` 연쇄가 끊기므로 당일 자동 연쇄를 종료한다. 활성 주기 중 실제 새 창을 여는 수동 워밍은 다음 창 충족으로 계산한다. 메뉴에 다음 실행·최근 결과·오류가 보이고 로그인 시 단일 인스턴스로 시작한다.
...
| `REQ-006` T-3분/T 확인 | `FLOW-003`, `FLOW-004`, `FLOW-006` | TC-CORE-001, TC-CORE-003, TC-CORE-004, TC-LIVE-001 |
