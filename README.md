...
검증 명령은 `swift test`, `swift build -c release`, `python3 scripts/verify-execution.py`, `bash scripts/verify-scheduler.sh late 147`입니다. 단위 테스트와 타이머 검증은 메모리 설정을 사용합니다. 실제 제품 서명·Keychain·설치 검증은 별도의 격리 환경에서 수행합니다.
