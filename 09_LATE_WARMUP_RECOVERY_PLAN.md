...
상태: 구현·자동 검증·서명/공증·설치 및 예약 복원 확인 완료. origin/dev 기준 codex/fix-late-warmup-recovery 브랜치에서 단일 에이전트로 수행했다. 결과는 [10 검증 기록](10_LATE_WARMUP_RECOVERY_VALIDATION.md)에 정리한다.
...
같은 실행일에서 직전에 확인한 `resets_at`과 60초 이내인 응답은 같은 창으로 취급한다. 이는 서버의 창 ID가 없는 현재 어댑터에서 미세한 시각 보정을 중복 집계하지 않기 위한 보수적 기준이다. 이전 확인 시각과 구버전 완료 횟수는 보존한다.
...
전송 여부가 불명확한 상태는 시간이 지났다는 이유나 앱 재시작만으로 해제하지 않는다. 확인 상한 이후에는 `결과 미확인`을 표시한다. 수동 버튼도 이 중복 방지 상태를 무시해 바로 다시 보내서는 안 된다. 명시적 재전송은 메뉴의 `미확인 전송을 해제하고 다시 워밍…`에서 별도 확인한다. `지금 워밍`은 미확인 기록을 유지한 채 조회를 재시도한다.
...
- 구버전 당일 `handledWindows`에는 성공·실패가 섞여 있으므로 정확한 성공 횟수를 임의 복원하지 않는다. 전환 기본안은 기존 집계를 당일 상한 계산용으로 보수적으로 보존하고 다음 실행일부터 새 집계를 적용하는 것이다. 이 경우 도입 당일 목표가 조기 종료될 수 있다는 한시적 제약을 기록한다. 구버전 JSON의 성공·실패·미확인·놓침 fixture로 이 전환 규칙을 검증한다.
...
완료 기준: 새 계약의 자동 회귀 및 release 빌드 통과. 자연스러운 DarkWake에서의 실제 워밍 결과는 사용자 담당으로 이번 완료 범위에서 제외한다.
...
- [x] 지연을 이유로 한 최초 5초·예약 3분 만료 제거
- [x] 앱 시작·타이머·복귀·수동 요청의 공통 판단과 단일 실행
- [x] 실제 확인한 창의 중복 없는 하루 집계와 구버전 기록 전환
- [x] 실제 리셋 기반 다음 예약, 가상 5시간 fallback 제거
- [x] 실패·미확인·재시도 상한·복구 동작과 상태 표시 검증
- [x] 위 검증표의 자동 회귀 및 release 빌드 통과
- [x] 관련 제품·흐름·테스트 문서 정합성 확보
- [x] 설치본 빌드 10 실행·기존 예약 복원 확인, 격리된 실제 타이머의 지연 처리·가짜 워밍·후속 예약 검증
