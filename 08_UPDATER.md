...
상태: 구현·자동 테스트·SDK 설치 경로·공개 피드 연결 완료. 현재 로컬 설치는 0.1.4 빌드 9이며 최종 공증은 프로필 접근 복구 후 진행한다. 제품 버전은 0.1.4로 고정한다.
...
- 원클릭 UI 어댑터의 승인 분기는 단위 테스트로 확인했다. 사용자 설치 앱의 화면을 자동 조작하는 도구가 접근 시간 초과를 반환해 실제 화면의 클릭 검증은 미실시다.
...
다음 배포 시에는 최종 공증된 0.1.4 DMG와 새로 생성한 appcast.xml을 같은 릴리스에 올려야 한다. 현재 빌드 9의 로컬 패키지는 `dist/ClaudeSessionWarmer-0.1.4-dev.dmg`이며 공개 배포용으로 사용하지 않는다. `dist/ClaudeSessionWarmer-0.1.4.dmg`에 남은 이전 빌드 8 아카이브를 최신 결과로 오인하지 않는다.
