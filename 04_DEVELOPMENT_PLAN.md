...
3. 첫 시각 3분 전에 CLI·인증·네트워크를 한 번 점검한다.
4. 첫 시각에 활성 창을 확인한다.
5. 활성 창이 없으면 PTY 최소 호출을 한 번 보내고, 이미 있으면 호출 없이 첫 창을 충족 처리한다.
6. 실제 `five_hour.resets_at`을 다음 실행 기준으로 저장한다.
7. 리셋 직후 다시 확인하여 필요할 때만 워밍한다.
8. 오류는 리셋 후 3분까지만 재시도한다.
9. 첫 창과 후속 두 창, 총 3개를 처리하면 그날 종료한다.
10. 다음 실행일의 첫 시각에 새 일일 주기를 시작한다.

`다음 워밍 건너뛰기`는 다음 창을 열지 않으므로, 그 실행일의 남은 자동 연쇄도 종료한다. 놓친 예약도 나중에 실행하지 않는다.
...
| TASK-002 | 첫 시각·요일·한국 공휴일·하루 3창 계산 | REQ-002, REQ-003, REQ-004, REQ-005, REQ-006 | 가짜 시간으로 정상일·휴일·3창 종료 테스트 통과 |
...
| TASK-005 | 예약과 중복 방지 연결 | REQ-004, REQ-005, REQ-006, REQ-007, REQ-011 | T-3분 점검, +3분 제한, no-catch-up, 하루 3창 동작 |
