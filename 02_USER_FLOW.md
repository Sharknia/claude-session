...
3. 새로고침 중 기존 Claude Code 자격증명을 읽을 때 macOS Keychain 접근 승인 대화상자를 표시한다. 앱은 토큰을 복사하거나 별도 저장하지 않는다.
...
사용자는 첫 워밍 시각 하나(예: 09:00), 실행 요일(예: 월·수·금), 대한민국 공휴일 제외 여부를 선택한다. 공휴일 제외는 기본으로 켜져 있다. 저장 시 앱은 다음 실행일을 계산해 메뉴에 표시한다. 설정을 변경하면 아직 시작하지 않은 예약만 새 정책으로 계산하고, 이미 충족된 창은 다시 실행하지 않는다.
