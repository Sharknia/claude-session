...
1. **T 정시 상태 확인**: 실행일·공휴일·인증 상태·CLI 경로·네트워크와 이미 충족된 창을 확인한다. 활성 창이면 호출 없이 `충족`으로 기록한다.
...
사용자는 draft 설정을 검토한 뒤 `저장`으로만 예약 재계산과 Keychain cache 준비를 적용한다.
